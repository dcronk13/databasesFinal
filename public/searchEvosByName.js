function searchPokeName() {
    //used by evolutions page to get value of name search string. sets the url so JS script can read and perform functions
    var poke_name_search_string  = document.getElementById('poke_name_search_string').value
    if (poke_name_search_string != ''){
        window.location = '/evolutions/search/' + encodeURI(poke_name_search_string)
    }
